...
df = pd.read_csv("../resources/purchase_history_apriori.csv", header=None)
df.isnull().sum()  # column wise missing values (NaN)
df.values.tolist()
T = []

# all the nan r in string format now. later added the nan to remove from list
for i in range(len(df)):
    T.append(
        [str(df.values[i, j]) for j in range(0, 4) if str(df.values[i, j]) != "nan"]
    )
# print(T)
